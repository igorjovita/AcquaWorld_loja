...
def select_reserva_id_titular(id_titular):
    mydb.connect()
    cursor.execute(
        f"SELECT r.id, r.nome_cliente, r.id_cliente, r.tipo, r.id_vendedor, r.receber_loja, r.valor_total, r.situacao, p.recebedor, p.pagamento from reserva as r LEFT JOIN pagamentos as p ON p.id_reserva = r.id where r.id_titular = {id_titular}")
